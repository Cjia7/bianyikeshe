...
void OBJECT::geneASM()
{
    ...
    QString MainProce = Rblocks.front().basic_block.front().arg1; // 主函数名
    // 遍历所有基本块
...
/*         posMap.clear(); */
        ...
        for (auto qt : block.basic_block)
        {
            if (isOperator1(qt.op))
            {
                ...
                if (qt.arg2 != "")
                    posMap[qt.arg2] = Position(qt.arg2, true);
...
/*                 saveR(block.curFun); */
            ...
            else if (qt.op == OPTIMIZE::PROCEDURE)
            {
                ...
                rec = {(int)(instVec.size() - 2), 0, block.curFun};
                record.push_back(rec);
            }
            else if (qt.op == OPTIMIZE::PROCEDURE_END)
            {
                if (block.curFun == MainProce)
                {
                    instVec.emplace_back("MOV", "AX", ",4C00H");
            ...
            else if (qt.op == OPTIMIZE::WRITE)
            {
                ...
                instVec.emplace_back("INT", "21H", "");
                instVec.emplace_back("MOV", "DL", ",13"); // 回车
                instVec.emplace_back("MOV", "AH", ",02H");
                instVec.emplace_back("INT", "21H", "");
                instVec.emplace_back("MOV", "DL", ",10"); // 换行
                instVec.emplace_back("MOV", "AH", ",02H");
                instVec.emplace_back("INT", "21H", "");
            }
    ...
    // 重排指令序列
    std::vector<Instruction> newVec = instVec; // 先保存原内容
    for (auto &rec : record)
    {
        newVec.insert(newVec.end(), instVec.begin() + rec.begin, instVec.begin() + rec.end);
    }

    /*     save_instVec("test.asm"); */
    for (auto rec = record.rbegin(); rec != record.rend(); ++rec)
    {
        newVec.erase(newVec.begin() + rec->begin, newVec.begin() + rec->end);
    }
    instVec = std::move(newVec);
    // 最后添加结束语句
    instVec.emplace_back("ENDS", "", "", "CSEG");
    instVec.emplace_back("END", MainProce, "", "");
}
...
void OBJECT::getR(const OPTIMIZE::QuadTuple &qtInput, QString &R, QString &B, QString &C, const QString &curFun)
{
    ...
    if (posMap.find(qt.arg1) != posMap.end() && !posMap[qt.arg1].isMem)
    {
        ...
        if (qt.isactive1 && (!posMap[qt.arg1].isMem || qt.arg1 != qt.result))
        {
            ...
            if (!emptyR.isEmpty())
            {
                ...
                RDL[emptyR] = qt.arg1;
                R = getAddr(qt.arg1, curFun);
                B = emptyR;
                if (qt.op != OPTIMIZE::OperatorType::ASSIGN)
                    C = getAddr(qt.arg2, curFun);
            ...
            else
            {
                ...
                R = posMap[qt.arg1].pos;
                B = getAddr(qt.arg1, curFun);
                if (qt.op != OPTIMIZE::OperatorType::ASSIGN)
                    C = getAddr(qt.arg2, curFun);
                RDL[posMap[qt.arg1].pos].clear();
                posMap[qt.arg1] = Position(qt.arg1, true);
    ...
    else
    {
        ...
        instVec.emplace_back("MOV", getAddr(obj, curFun), "," + objPos);
        RDL[objPos].clear();
